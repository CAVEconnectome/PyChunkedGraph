...
def auth_required(func=None, *, required_permission=None, public_table_key=None, public_node_key='elelel', service_token='None'):
    print(f"auth_required 0:{required_permission} 1:{public_table_key} 2:{public_node_key} 3:{service_token}")
    print("func")
    print(func)
    # if func is None:
    ...
    def decorator(f):
        print(f"auth_required-decorator 0:{required_permission} 1:{public_table_key} 2:{public_node_key} 3:{service_token}")


        testtesttest = service_token

        hhh = copy.deepcopy(service_token)

        boop = 'beeboop'

        print(f"hhh outer: {hhh}")

        # @wraps(f)
        def decorated_function(*args, **kwargs):
            print(f"boop: {boop}")
            print(f"TTT1: {testtesttest}")
            print(f"hhh: {hhh}")
            print(f"public_node_key: {public_node_key}")
            ...
            print(f"auth_required-decorated_function 0:{required_permission} 1:{public_table_key} 2:{public_node_key} 3:{service_token}")
            ...
            def lazy_check_public_access():
                print(f"lazy_check_public_access 0:{service_token} - 1:{required_permission}")
                # if flask.g.public_access_cache is None:
                print(f"lazy_check_public_access updating_cache 0:{service_token} - 1:{required_permission}")
                    # if service_token and required_permission is not 'edit':
                    #     if public_node_key is not None:
                    #         print("lazy_check_public_access case1")
                    ...
                    #         print("lazy_check_public_access case2")
                    #         flask.g.public_access_cache = table_has_public(kwargs.get(public_table_key), service_token)
                    #     else:
                    #         print("lazy_check_public_access case3")
                    #         flask.g.public_access_cache = False
                    # else:
                    #     print("lazy_check_public_access case4")
                    #     flask.g.public_access_cache = False

                return False #flask.g.public_access_cache
    ...
    if func:
        ...
        print("we dont have func")
        return decorator


def auth_required_extra(required_permission=None, public_table_key=None, public_node_key='elelel', service_token='None'):
    print(f"auth_required_extra 0:{required_permission} 1:{public_table_key} 2:{public_node_key} 3:{service_token}")

    def decorator(f):
        print(f"auth_required_extra-decorator 0:{required_permission} 1:{public_table_key} 2:{public_node_key} 3:{service_token}")
        ...
        def decorated_function(*args, **kwargs):
            ...
            print(f"auth_required_extra-decorated_function 0:{required_permission} 1:{public_table_key} 2:{public_node_key} 3:{service_token}")
            ...
            # flask.g.public_access_cache = None
            def lazy_check_public_access():
                print(f"auth_required_extra-lazy_check_public_access 0:{service_token} - 1:{required_permission}")
                # if flask.g.public_access_cache is None:
                print(f"auth_required_extra-lazy_check_public_access updating_cache 0:{service_token} - 1:{required_permission}")
                    # if service_token and required_permission is not 'edit':
                    #     if public_node_key is not None:
                    #         print("lazy_check_public_access case1")
                    #         flask.g.public_access_cache = is_root_public(kwargs.get(public_table_key), kwargs.get(public_node_key), service_token)
...
def auth_requires_permission(required_permission, public_table_key=None, public_node_key=None, service_token=None):
    print(f"auth_requires_permission 0:{required_permission} 1:{public_table_key} 2:{public_node_key} 3:{service_token}")
    def decorator(f):
        @wraps(f)
        @auth_required_extra(required_permission=required_permission, public_table_key=public_table_key, public_node_key=public_node_key, service_token=service_token)
