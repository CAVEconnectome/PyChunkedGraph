...
def handle_merge(table_id):
    ...
    if len(ret.new_lvl2_ids) > 0:
        t = threading.Thread(
            target=_remeshing, args=(cg.get_serialized_info(), lvl2_nodes)
        )
        t.start()

    return {"new_rootIds": [new_root]}
...
def handle_undo(table_id):
    NotImplementedError("This feature is unavailable")
    # current_app.table_id = table_id
    #
    # data = json.loads(request.data)
    # user_id = str(g.auth_user["id"])
