...
sys.path.insert(0, '/code/pychunkedgraph/backend') #Include Sven's pychunkedgraph code
#sys.path.insert(0, '/usr/people/zashwood/Documents/PyChunkedGraph/src/backend') #Include Sven's backend code
...
#Redis host
redis_conn = redis.StrictRedis(host="redis", port=6379, charset="utf-8", decode_responses=True)
...
def handle_merge():
	# Record time when request received by master_dep
    time_server_start = str(datetime.datetime.now())
...
if __name__ == '__main__':
    ...
    topic_path = publisher.topic_path('neuromancer-seung-import', 'backend')
