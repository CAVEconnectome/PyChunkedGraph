...
def run_baseline_stitch(graph_id: str, atomic_edges: np.ndarray, do_sanity_check: bool = True) -> dict:
    """
    ...
    print(f"  [baseline] stitch ({len(atomic_edges)} edges)...")
    t0 = time.time()
    result = cg.add_edges(
        user_id="test",
    ...
    print(f"  [baseline] stitch: {elapsed:.1f}s, {len(new_roots)} roots")
    ...
    print(f"  [baseline] structure: {time.time() - t0:.1f}s")
