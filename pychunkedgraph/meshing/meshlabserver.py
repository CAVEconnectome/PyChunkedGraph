...
def run_meshlab_script(script_name, arg_dict):
    ...
    """
    arg_string = "".join(["-{0} {1} ".format(k, arg_dict[k]) for k in arg_dict.keys()])
    command = (
        "xvfb-run --auto-servernum --server-num=1 meshlabserver -s {0}/{1} {2}".format(
            path_to_scripts, script_name, arg_string
        )
    )
...
def _run_meshlab_script_on_dir_thread(args):
    ...
    for path in path_block:
        out_path = "{}/{}{}.obj".format(
            out_dir, "".join(os.path.basename(path).split(".")[:-1]), suffix
        )
...
def run_meshlab_script_on_dir(
    script_name, in_dir, out_dir, suffix, arg_dict={}, n_threads=1
):
    ...
    if n_threads == 1:
        for args in multi_args:
            _run_meshlab_script_on_dir_thread(args)
    else:
        with ProcessPoolExecutor(max_workers=n_threads) as executor:
            list(executor.map(_run_meshlab_script_on_dir_thread, multi_args))
