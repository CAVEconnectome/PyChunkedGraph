...
def add_atomic_edges_in_chunks(
    ...
    for key in edge_id_keys:
        edge_id_dict[key] = np.concatenate(
            edge_id_dict.get(key, empty_edges_array.copy(), empty_edges_array.copy())
        )
        n_edge_ids += len(edge_id_dict[key])

    for key in edge_aff_keys:
        edge_aff_dict[key] = np.concatenate(
            edge_aff_dict.get(key, empty_edges_array.copy(), empty_edges_array.copy())
        )
    ...
    parent_chunk_id = cg.get_chunk_id(layer=2, *cg.get_chunk_coordinates(chunk_id))
    ...
    for i_cc, cc in enumerate(ccs):
        ...
        u_chunk_ids = np.unique([cg.get_chunk_id(n) for n in node_ids])

        if len(u_chunk_ids) > 1:
            cg.logger.error(f"Found multiple chunk ids: {u_chunk_ids}")
            raise Exception()
