...
def mincut_nx(
    ...
    for cc in ccs:
        ...
        if not np.any(np.in1d(sources, cc_list)) or not np.any(np.in1d(sinks, cc_list)):
            weighted_graph.remove_nodes_from(cc)

    r_flow = edmonds_karp(weighted_graph, sinks[0], sources[0])
    cutset = minimum_st_edge_cut(weighted_graph, sources[0], sinks[0], residual=r_flow)
    ...
    remapped_cutset = []
    for cut in cutset:
        pre_cut = remapping[cut[0]] if cut[0] in remapping else [cut[0]]
        post_cut = remapping[cut[1]] if cut[0] in remapping else [cut[1]]
...
def mincut_graph_tool(
    ...
    comb_affs = np.concatenate(
        [affs, [float_max,] * (len(sink_edges) + len(source_edges))]
    )

    # To make things easier for everyone involved, we map the ids to
    # [0, ..., len(unique_ids) - 1]
    # Generate weighted graph with graph_tool
    weighted_graph, cap, gt_edges, unique_ids = flatgraph.build_gt_graph(
    ...
    if len(ccs) > 1:
        for cc in ccs:
            ...
            # remove its nodes from the mincut computation
            if not (
                np.any(np.in1d(source_graph_ids, cc))
                and np.any(np.in1d(sink_graph_ids, cc))
            ):
                for node_id in cc:
    ...
    # Compute mincut
    src, tgt = (
        weighted_graph.vertex(source_graph_ids[0]),
    ...
    if DEBUG_MODE:
        ...
        for i_cc in np.unique(part.a):
            ...
            cc_list = unique_ids[np.array(np.where(part.a == i_cc)[0], dtype=np.int)]
    ...
    for s, t in flatgraph.remap_ids_from_graph(cut_edge_set, unique_ids):
        s = remapping[s] if s in remapping else [s]
        t = remapping[t] if t in remapping else [t]
...
def mincut(
    ...
    :return: m x 2 array of uint64s
        edges that should be removed
    """
    return mincut_graph_tool(
        edges=edges, affs=affs, sources=sources, sinks=sinks, logger=logger
    )
