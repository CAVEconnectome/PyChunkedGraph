...
def get_latest_edges(
    ...
    def _get_l2chunkids_along_boundary(mlayer: int, coord_a, coord_b, padding: int = 0):
        ...
        bounds_a = chunk_utils.get_bounding_children_chunks(cg.meta, mlayer, coord_a, 2)
        bounds_b = chunk_utils.get_bounding_children_chunks(cg.meta, mlayer, coord_b, 2)
    ...
    for edge_layer, _edge in zip(edge_layers, stale_edges):
        ...
        if l2ids_a.size == 0 or l2ids_b.size == 0:
            logging.info("None found, expanding search with padding.")
            mlayer, l2ids_a, l2ids_b = _get_filtered_l2ids(node_a, node_b, padding=2)
            logging.info(f"Found {l2ids_a.size} and {l2ids_b.size}.")
