...
def update_chunk(cg: ChunkedGraph, chunk_coords: list[int], debug: bool = False):
    ...
    nodes = np.array(nodes, dtype=np.uint64)
    node_parents = cg.get_parents(nodes, fail_to_zero=True)
    invalid_mask = node_parents != 0
    nodes = nodes[invalid_mask]
    nodes_ts = nodes_ts[invalid_mask]
